...
struct module_exports exports = {
    "database_hash", cmds, /* Exported functions */
...
int rfc1918address(str *address) {
  ...
  LDEBUG("CHECKING IP RFC [%s] - [%u], [%u], [%d]", in4_string, in4_addr,
         netaddr, result);

  for (i = 0; rfc1918nets[i].name != NULL; i++) {
    LDEBUG("CHECKING RFC IN ADR:[%u],MASK[%u] RES:[%u]", rfc1918nets[i].address,
           rfc1918nets[i].mask, (netaddr & rfc1918nets[i].mask));
...
int w_check_rtcp_ipport(msg_t *msg) {
  ...
  snprintf(callid, sizeof(callid), "%.*s", msg->sip.callId.len,
           msg->sip.callId.s);
  ...
  for (i = 0; i < msg->sip.mrp_size; i++) {
    ...
    if (mp->rtcp_ip.len > 0 && mp->rtcp_ip.s) {
      snprintf(ipptmp, sizeof(ipptmp), "%.*s:%d", mp->rtcp_ip.len,
               mp->rtcp_ip.s, mp->rtcp_port);
      ...
      if (nat_mode == 1 && rfc1918address(&mp->rtcp_ip) == 1) {
        LDEBUG("Detected NAT IP from RFC1918 [%.*s] changing to [%s]",
               mp->rtcp_ip.len, mp->rtcp_ip.s, msg->rcinfo.src_ip);
        snprintf(ipptmp, sizeof(ipptmp), "%s:%d", msg->rcinfo.src_ip,
                 mp->rtcp_port);
      }
...
void print_ipports() {
  ...
  HASH_ITER(hh, ipports, s, tmp) { LDEBUG("NAME IPPORTS: %s", s->name); }
    ...
    if (settings != NULL) {
      ...
      while (params) {
        ...
        if (params->attr[0] != NULL) {
          ...
          /* cache */
          if (!strncmp(key, "timer-timeout", 13) && atoi(value) > 200)
            timer_timeout = atoi(value);
          else if (!strncmp(key, "rtcp-timeout", 12) && atoi(value) > 80)
            rtcp_timeout = atoi(value);
          else if (!strncmp(key, "nat-mode", 8))
            nat_mode = atoi(value);
          else if (!strncmp(key, "debug", 5))
            debug_param = atoi(value);
        }
...
static int free_profile(unsigned int idx) {
  ...
  if (profile_database[idx].name)
    free(profile_database[idx].name);
  if (profile_database[idx].description)
    free(profile_database[idx].description);
