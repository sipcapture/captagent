...
static void append_e164(srjson_doc_t *doc, const char *name, const uint8_t *data, uint8_t len, const uint8_t type)
{
	...
	if (!obj) {
		LERR("Can not allocate json object for %s\n", name);
		return;
	}
...
static void append_cause(srjson_doc_t *doc, const char *name, const uint8_t *data, uint8_t len)
{
	...
	if (len < 2) {
		LERR("Not enough data for cause\n");
		return;
	}
...
static void append_event_information(srjson_doc_t *doc, const char *name, const uint8_t *data, uint8_t len)
{
	...
	if (len < 1) {
		LERR("Not enough data for event information\n");
		return;
	}
...
static void append_hop_counter(srjson_doc_t *doc, const char *name, const uint8_t *data, uint8_t len)
{
	...
	if (len < 1) {
		LERR("Not enough data for hop counter\n");
		return;
	}
...
static void append_calling_party_category(srjson_doc_t *doc, const uint8_t *data, uint8_t len)
{
	...
	if (len < 1) {
		LERR("Not enough data for transport medium requirement\n");
		return;
	}

	obj = srjson_CreateObject(doc);
	if (!obj) {
		LERR("Can not allocate json object for transport medium requirement\n");
		return;
	}
...
static void append_nci(srjson_doc_t *doc, const char *name, const uint8_t *data, uint8_t len)
{
	...
	if (len != 1) {
		LERR("Unpexected size(%u) for nature of connection indicators\n", len);
		return;
	}
...
static void append_forward_call(srjson_doc_t *doc, const char *name, const uint8_t *data, uint8_t len)
{
	...
	if (len != 2) {
		LERR("Unpexected size(%u) for forward call indicators\n", len);
		return;
	}
...
static void append_transmission_medium(srjson_doc_t *doc, const uint8_t *data, const uint8_t len)
{
	...
	if (len != 1) {
		LERR("Unpexected size(%u)\n", len);
		return;
	}

	obj = srjson_CreateObject(doc);
	if (!obj) {
		LERR("Can not allocate json object\n");
		return;
	}
...
static void append_user_information(srjson_doc_t *doc, const uint8_t *data, const uint8_t len)
{
	...
	if (transfer_rate == 0x18) {
		if (len < 4) {
			LERR("Insufficient size(%u) for multirate\n", len);
			return;
		}
	...
	*cic = parse_cic(data);
	...
	if (msg_class->fixed_ies) {
		...
		while (1) {
			...
			if (left < fixed->len) {
				LERR("ISUP fixed too short %zu vs. %un", left, fixed->len);
				return -3;
			}
	...
	if (msg_class->variable_ies) {
		...
		while (1) {
			...
			if (ie_left < ie_data[0]) {
				LERR("ISUP no space for len %zu vs. %u\n", ie_left, ie_data[0]);
				return -1;
			}
			ie_left -= ie_data[0];
			ie_data += ie_data[0];
			...
			ie_len = ie_data[0];
			if (ie_left < ie_len + 1) {
				LERR("ISUP no space for data %zu vs. %u\n", ie_left, ie_len + 1);
				return -1;
			}
	...
	if (msg_class->has_optional) {
		...
		if (opt_left < 1) {
			LERR("ISUP no space for optional ptr\n");
			return -1;
		}
		if (opt_left < opt_data[0]) {
			LERR("ISUP optional beyond msg %zu vs. %u\n", opt_left, opt_data[0]);
			return -1;
		}
		...
		while (opt_left > 0) {
			...
			if (opt_left < len) {
				LERR("ISUP no space optional data %zu vs. %zu\n", opt_left, len);
				return -1;
			}
