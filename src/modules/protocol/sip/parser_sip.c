...
bool getTag(str *hname, char *uri, int len) {
        ...
        if((last_offset - first_offset) < 5) return FALSE;
...
int parse_message(char *message, unsigned int blen, unsigned int* bytes_parsed, sip_msg_t *psip, unsigned int type) {
	...
	if (!memcmp("SIP/2.0 ", tmp, 8)) {
		...
		if (!memcmp(tmp, INVITE_METHOD, INVITE_LEN))
		{
			...
			allowRequest = TRUE; allowPai = TRUE;
		}
	...
	for (; *c && c - message < new_len; c++) {
		...
		if (*c == '\r' && *(c + 1) == '\n') { /* end of this line */
			...
			else if(parseVIA && ((*tmp == 'V' || *tmp == 'v') && (*(tmp+1) == 'i' || *(tmp+1) == 'i') && *(tmp+VIA_LEN) == ':')) {
					set_hname(&psip->via, (offset - last_offset - VIA_LEN), tmp+VIA_LEN);
					continue;
			}
			else if(parseContact && ((*tmp == 'm' && *(tmp+1) == ':') ||
					((*tmp == 'C' || *tmp == 'c') && ( *(tmp+5) == 'C' || *(tmp+5) == 'c') && *(tmp+CONTACT_LEN) == ':')))
			{
				 	if(*(tmp+1) == ':') header_offset = 1;
				    else header_offset = CONTACT_LEN;
            ...
            if(allowPai) {
         	   ...
         	   if(((*tmp == 'P' || *tmp == 'p') && (*(tmp+2) == 'P' || *(tmp+2) == 'p') && ( *(tmp+13) == 'i' || *(tmp+13) == 'I')
         		...
         		else if(((*tmp == 'P' || *tmp == 'p') && (*(tmp+2) == 'A' || *(tmp+2) == 'a') && ( *(tmp+13) == 'i' || *(tmp+13) == 'I')
     	    ...
     	    if(allowRequest)
            {
						if ((*tmp == 'f' && *(tmp + 1) == ':')	|| ((*tmp == 'F' || *tmp == 'f')	&& (*(tmp + 3) == 'M' || *(tmp + 3) == 'm')
										&& *(tmp + FROM_LEN) == ':'))
						{

							if(*(tmp+1) == ':') header_offset = 1;
							else header_offset = FROM_LEN;
