...
int parseSdpARtpMapLine(codecmap_t *cp, char *data, int len) {
    ...
    if (*c == '\r' && *(c + 1) == '\n') { /* end of this line */
      ...
      if ((*tmp == 'm' && *(tmp + 1) == '=')) {
        ...
        if (set_ip == 1)
          ...
          if (psip->mrp_size > 0) {
            psip->mrp[psip->mrp_size].media_ip.s =
                psip->mrp[psip->mrp_size - 1].media_ip.s;
            psip->mrp[psip->mrp_size].media_ip.len =
                psip->mrp[psip->mrp_size - 1].media_ip.len;
            mp = &psip->mrp[psip->mrp_size];
          }
        }
      ...
      /* a=rtcp-mux */
               ...
               !memcmp(tmp + 2, "rtcp-mux", 8)) {
        if (mp == NULL) {
          printf("BAD SDP. Couldn't parse it [RTCP-MUX]!\n");
          return 0;
        }

        /* RTCP mux means RTP and RTCP share the same port. */
        mp->rtcp_port = mp->media_port;

        if (mp->rtcp_ip.len == 0 && mp->media_ip.len > 0) {
          mp->rtcp_ip = mp->media_ip;
        }
      }
      /**/
      /* a=rtcp:53020 IN IP4 126.16.64.4 */
...
method_t getMethodType(char *s, int len) {
...
bool splitCSeq(sip_msg_t *sipStruct, char *s, int len) {
...
int parse_message(char *message, unsigned int blen, unsigned int *bytes_parsed,
                  sip_msg_t *psip, unsigned int type) {
  ...
  if (!memcmp("SIP/2.0 ", tmp, 8)) {
    ...
    if ((pch = strchr(tmp + 1, ' ')) != NULL) {
      ...
      if ((ped = strchr(pch + 1, ' ')) != NULL) {
        ...
        /* extract user */
        getUser(&psip->ruriUser, &psip->ruriDomain, psip->requestURI.s,
                psip->requestURI.len);
      }
    }
  }
  ...
  for (; *c && c - message < new_len; c++) {
    ...
    if (*c == '\r' && *(c + 1) == '\n') { /* end of this line */
      ...
      if ((*tmp == 'i' && *(tmp + 1) == ':') ||
        ...
        set_hname(&psip->callId, (offset - last_offset - header_offset),
                  tmp + header_offset);
        continue;
      }
      /* Content-Length */
      if ((*tmp == 'l' && *(tmp + 1) == ':') ||
        ...
        if (set_hname(&psip->cSeq, (offset - last_offset - CSEQ_LEN),
                      tmp + CSEQ_LEN)) {
      ...
      /* content type  Content-Type: application/sdp  CONTENTTYPE_LEN */
        ...
        set_hname(&psip->contactURI, (offset - last_offset - header_offset),
                  tmp + header_offset);
        continue;
      } else if ((*tmp == 'f' && *(tmp + 1) == ':') ||
                 ((*tmp == 'F' || *tmp == 'f') &&
        ...
        if (set_hname(&psip->fromURI, (offset - last_offset - header_offset),
                      tmp + header_offset)) {
          psip->hasFrom = TRUE;
          if (!(psip->fromURI.len == 0) &&
              getTag(&psip->fromTag, psip->fromURI.s, psip->fromURI.len)) {
            psip->hasFromTag = TRUE;
          }
          /* extract user */
          getUser(&psip->fromUser, &psip->fromDomain, psip->fromURI.s,
                  psip->fromURI.len);
        }
        ...
        if (set_hname(&psip->toURI, (offset - last_offset - header_offset),
                      tmp + header_offset)) {
          psip->hasTo = TRUE;
          if (!(psip->toURI.len == 0) &&
              getTag(&psip->toTag, psip->toURI.s, psip->toURI.len)) {
            psip->hasToTag = TRUE;
          }
          /* extract user */
          getUser(&psip->toUser, &psip->toDomain, psip->toURI.s,
                  psip->toURI.len);
        }
        continue;
      }
      ...
      /* UserAgent */
        ...
        set_hname(&psip->userAgent, (offset - last_offset - header_offset),
                  tmp + header_offset);
        continue;
      }
      ...
      if (allowPai) {
        ...
        if (((*tmp == 'P' || *tmp == 'p') &&
          ...
          if (set_hname(&psip->pidURI,
                        (offset - last_offset - PPREFERREDIDENTITY_LEN),
                        tmp + PPREFERREDIDENTITY_LEN)) {
            psip->hasPid = TRUE;
            /* extract user */
            getUser(&psip->paiUser, &psip->paiDomain, psip->pidURI.s,
                    psip->pidURI.len);
          }
          continue;
        } else if (customHeaderMatch != NULL &&
                   (*(tmp + customHeaderLen) == ':' &&
                    !strncmp(tmp, customHeaderMatch, customHeaderLen))) {
          if (set_hname(&psip->customHeader,
                        (offset - last_offset - customHeaderLen),
                        tmp + customHeaderLen)) {
...
int light_parse_message(char *message, unsigned int blen,
                        unsigned int *bytes_parsed, sip_msg_t *psip) {
  unsigned int new_len = blen;
  int header_offset = 0;
