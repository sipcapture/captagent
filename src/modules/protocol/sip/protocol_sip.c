...
struct module_exports exports = {
    "protocol_sip", cmds, /* Exported functions */
    load_module,          /* module initialization function */
    unload_module,  description, statistic, serial_module};
...
int w_send_reply(msg_t *_m) { return send_sip_reply(_m, 200, "OK"); }

int w_parse_sip(msg_t *_m) { return parse_sip(_m, 1); }

int w_light_parse_sip(msg_t *_m) { return light_parse_sip(_m); }

int w_parse_full_sip(msg_t *_m) { return parse_sip(_m, 2); }

int w_clog(msg_t *_m, char *param1, char *param2) {

  if (param1[0] == 'E' || param1[0] == 'e')
    LERR("%s\n", param2);
  else if (param1[0] == 'N' || param1[0] == 'n')
    LNOTICE("%s\n", param2);
  else
    LDEBUG("%s\n", param2);
...
int w_set_tag(msg_t *_m, char *param1, char *param2) {
  ...
  _m->rcinfo.tags.len += snprintf(
      _m->rcinfo.tags.s + _m->rcinfo.tags.len,
      sizeof(_m->rcinfo.tags.s) - _m->rcinfo.tags.len, "%s=%s", param1, param2);

  return 1;
}

static char *find_sip_header(const char *data, unsigned int len,
                             const char *header_name, int *value_len) {
...
int w_header_check(msg_t *_m, char *param1, char *param2) {
  if (!strncmp("User-Agent", param1, strlen("User-Agent")) ||
      !strncmp("useragent", param1, strlen("useragent"))) {
    ...
    int value_len = 0;
    char *value =
        find_sip_header((const char *)_m->data, _m->len, param1, &value_len);
    if (value != NULL) {
      size_t param2_len = strlen(param2);
      if (value_len > 0 && (size_t)value_len >= param2_len &&
          strncmp(value, param2, param2_len) == 0) {
...
int w_header_reg_match(msg_t *_m, char *param1, char *param2) {
  ...
  if (param2 != NULL)
    index = get_pcre_index_by_name(param2);

  if (!strncmp("User-Agent", param1, strlen("User-Agent")) ||
      !strncmp("useragent", param1, strlen("useragent"))) {
    if (_m->sip.userAgent.s && _m->sip.userAgent.len > 0) {
      if ((re_match_func(pattern_match[index], _m->sip.userAgent.s,
                         _m->sip.userAgent.len)) == 1) {
        LDEBUG(">>>> UserAgent SIP matched: [%.*s]", _m->sip.userAgent.len,
               _m->sip.userAgent.s);
        return 1;
      }
    }
    ...
    if (_m->sip.customHeader.s && _m->sip.customHeader.len > 0) {
      if ((re_match_func(pattern_match[index], _m->sip.customHeader.s,
                         _m->sip.customHeader.len)) == 1) {
        LDEBUG(">>>> Custom SIP matched: [%.*s]", _m->sip.customHeader.len,
               _m->sip.customHeader.s);
        return 1;
      }
    }
  } else if (!strncmp("body", param1, strlen("body")) ||
             !strncmp("raw", param1, strlen("raw"))) {
    if (_m->data && _m->len > 0) {
    ...
    if (value != NULL && value_len > 0) {
      if ((re_match_func(pattern_match[index], value, value_len)) == 1) {
        LDEBUG(">>>> Header [%s] SIP matched: [%.*s]", param1, value_len,
               value);
        return 1;
      }
    }
...
int w_sip_check(msg_t *_m, char *param1, char *param2) {
  ...
  if (!strncmp("method", param1, strlen("method"))) {
    if (param2 != NULL && _m->sip.methodString.s &&
        _m->sip.methodString.len > 0 &&
        !strncmp(_m->sip.methodString.s, param2, strlen(param2))) {
      ret = 1;
    }
  } else if (!strncmp("rmethod", param1, strlen("rmethod"))) {
    if (param2 != NULL && _m->sip.cSeqMethodString.s &&
        _m->sip.cSeqMethodString.len > 0 &&
  ...
  } else if (!strncmp("user_agent_prefix", param1,
                      strlen("user_agent_prefix"))) {
    if (startwith(&_m->sip.userAgent, param2)) {
      ret = 1;
    }
  } else if (!strncmp("user_agent_suffix", param1,
                      strlen("user_agent_suffix"))) {
...
int send_sip_reply(msg_t *_m, int code, char *description) {
  ...
  n = snprintf(reply, sizeof(reply),
               ...
               code, description, _m->sip.via.len, _m->sip.via.s,
               _m->sip.fromURI.len, _m->sip.fromURI.s, _m->sip.toURI.len,
               _m->sip.toURI.s, "Fg2Uy0r7geBQF", _m->sip.contactURI.len,
               _m->sip.contactURI.s, _m->sip.callId.len, _m->sip.callId.s,
  ...
  sendto(*_m->rcinfo.socket, reply, n, 0, (struct sockaddr *)&cliaddr,
         sizeof(cliaddr));
...
int reload_config(char *erbuf, int erlen) {
  ...
  snprintf(module_config_name, 500, "%s/%s.xml", global_config_path,
           module_name);
...
int8_t re_match_func(pcre *pattern, char *data, uint32_t len) {
  ...
  if (pattern && len > 0) {
    ...
    pcreExtRet = pcre2_match(pattern, (PCRE2_SPTR)escapeData,
                             strlen(escapeData), 0, 0, match_data, NULL);
...
#else
    pcreExtRet = pcre_exec(pattern, 0, escapeData, strlen(escapeData), 0, 0,
                           subStrVec, 30);
...
int set_value(unsigned int idx, msg_t *msg) { return 1; }
...
profile_protocol_t *get_profile_by_name(char *name) {
  ...
  if (profile_size == 1)
    return &profile_protocol[0];

  for (i = 0; i < profile_size; i++) {

    if (!strncmp(profile_protocol[i].name, name,
                 strlen(profile_protocol[i].name))) {
...
void free_regexp() {
  ...
  unsigned int i = 0;
  for (i = 0; i < profile_size; i++) {
    if (regexpIndexName[i])
      free(regexpIndexName[i]);
#ifdef USE_PCRE2
    if (pattern_match[i])
      pcre2_code_free(pattern_match[i]);
#else
    ...
    if (settings != NULL) {
      ...
      while (params) {
        ...
        if (params->attr[0] != NULL) {
          ...
          if (params->attr[2] && params->attr[3] &&
              !strncmp(params->attr[2], "value", 5)) {
            value = params->attr[3];
          } else {
          ...
          if (!strncmp(key, "ignore", 6))
            ...
            if (regexpIndex < MAX_REGEXP_INDEXES) {
              ...
              pattern_match[regexpIndex] =
                  pcre2_compile((PCRE2_SPTR)regexpIndexName[regexpIndex],
                                PCRE2_ZERO_TERMINATED, pcre_options, &errcode,
                                &erroffset, NULL);
...
#else
              pattern_match[regexpIndex] =
                  pcre_compile(regexpIndexName[regexpIndex], pcre_options,
                               (const char **)&re_err, &err_offset, 0);
              if (!pattern_match[regexpIndex]) {
                LERR("pattern_match I:[%d] compile failed: %s\n", regexpIndex,
                     re_err);
              } else
...
static uint64_t serial_module(void) { return module_serial; }

static int free_profile(unsigned int idx) {

  /*free profile chars **/

  if (profile_protocol[idx].name)
    free(profile_protocol[idx].name);
  if (profile_protocol[idx].description)
    free(profile_protocol[idx].description);
  if (profile_protocol[idx].ignore)
    free(profile_protocol[idx].ignore);
