...
# install libs
cd $TMP_DIR
apt-get -y update && apt-get -y install git make gcc libexpat-dev libpcap-dev libjson-c-dev libuv1-dev libtool automake autoconf flex bison libgcrypt20 libgcrypt20-dev libssl-dev
...
fpm -s dir -t deb -C /tmp/captagent_install --name captagent --version $CODEVERSION --iteration 1 \
    -p "captagent_tls-${CODEVERSION}.${OS}.amd64.deb" \
